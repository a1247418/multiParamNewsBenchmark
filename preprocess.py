...
if __name__ == '__main__':
    ...
    words = set()
    for topic_id in range(config.n_topics):
        words_for_topic = lda.get_topic_terms(topicid=topic_id, topn=config.n_top_words_per_topic)
    ...
    if config.normalize_outputs:
        for i in range(n_words_in_corpus):
    ...
    to_save = {
        ...
        'z': corpus_z,
        'dim_x': len(words),
        'dim_z': config.n_topics
    }

    pickle.dump(to_save, open(config.lda_file, "wb"))

    print("Done preprocessing adapted News benchmark.")
